...
for CliName in main; do
  for GOOS in darwin linux windows; do
    ...
    then
       ...
       go build -o bin/enumcli-$GOOS-$GOARCH.exe cmd/$CliName/*.go
    else
       echo "Building $GOOS-$GOARCH"
      go build -o bin/enumcli-$GOOS-$GOARCH cmd/$CliName/*.go
    fi
    done
  done
done' &&
  ...
  echo "running : ${BinDir}/enumcli-linux-amd64" &&
  "$BinDir"/enumcli-linux-amd64 &&
  echo "" &&
